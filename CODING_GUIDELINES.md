...
**Property tests vs. parameterized example tests — name the file for what it actually does.** A property test uses a generator that explores an effectively open domain — `Arb.string()`, `Arb.int()`, `Arb.list()`, `Arb.map()`, `Arb.bind()`, or a custom function returning `Arb<T>` — checked against an independently computed oracle (a regex, a hand-rolled reference implementation, a construction-tagged ground truth). `Arb.of(handPickedList)` wrapped in `forAll`/`checkAll` is not that — it's iterating a fixed list of examples you chose yourself, however many cases are in it. That's a **parameterized example test**, and it belongs in a plain `@Test` with a `forEach`/`shouldBe` loop, not `runBlocking { forAll(Arb.of(...)) { ... } }` — the kotest property machinery adds nothing here and its generic shrink-failure output is worse than a normal assertion failure with actual/expected values.

Name the file accordingly: `*PropertyTest.kt` only if it contains a real generator, `*Test.kt` otherwise. This isn't cosmetic — a `*PropertyTest.kt` file that turns out to be a fixed example list under the hood gives false confidence that the classification logic was actually explored across its input space, when only a handful of hand-picked cases were checked. `core/src/test/kotlin/com/github/tvinke/algorilla/util/WordBoundaryMatchingPropertyTest.kt` is the canonical example of a real one: it generates strings from disjoint letter alphabets via `Arb.string()`/`checkAll` and checks the result against an independently computed oracle (a regex for the prefix case, a construction-tagged ground truth for suffix/contains), rather than asserting against a list of inputs picked by the person writing the test.

A small, deliberately curated set of *named* real-world scenarios (not a broad generated domain) is a legitimate design choice, not a lesser version of a property test — but it's still an example test and should be named and treated as one. `checkPropertyTestNaming` in `core/build.gradle.kts` enforces this at build time (part of `check`): any `*PropertyTest.kt` file with no real generator fails the build unless it's on that task's small, reasoned allowlist.
