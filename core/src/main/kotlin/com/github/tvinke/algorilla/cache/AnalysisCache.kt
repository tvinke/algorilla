 ...
 *
 * The cache includes a version stamp derived from the semantics YAML files, the algorilla
 * build version, and the resolved rule config (enabled/disabled rules, confidence threshold,
 * per-rule overrides). Any of those changing what a rule would report for an unchanged file
 * busts the whole cache — a Kotlin-only rule fix or a `.algorilla.yml` toggle doesn't touch
 * the semantics YAML, so relying on that alone left stale findings served after either.
 */
public class AnalysisCache(
    private val baseDir: File,
    config: AnalysisConfig? = null,
    private val cacheVersion: String = computeCacheVersion(config),
) {
    ...
    public fun load(): Map<String, CachedFileEntry> {
        ...
        return try {
            ...
            if (data.version != cacheVersion) {
                logger.info { "Cache version mismatch (${data.version} vs $cacheVersion), re-analyzing all files" }
                return emptyMap()
            }
    ...
    public fun save(entries: List<CachedFileEntry>) {
        ...
        val data = CacheData(version = cacheVersion, files = entries)
...
private const val VERSION_RESOURCE = "algorilla-version.properties"
private const val UNKNOWN_BUILD_VERSION = "dev"
private const val NO_CONFIG_FINGERPRINT = "no-config"

/**
 * Combines the semantics YAML bytes, the algorilla build version, and the resolved rule
 * config into a single cache-version stamp. Any of the three changing invalidates the cache.
 */
private fun computeCacheVersion(config: AnalysisConfig?): String {
    val digest = MessageDigest.getInstance("SHA-256")
    ...
    digest.update(buildVersion().toByteArray())
    digest.update(configFingerprint(config).toByteArray())
    return digest.digest().joinToString("") { "%02x".format(it) }.take(HASH_PREFIX_LENGTH)
}

/**
 * Reads the algorilla build version from the classpath resource generated by `:cli`'s
 * `generateVersionFile` task (same resource [AlgorillaCommand.getVersion] reads). Falls back
 * to a stable constant when it's absent — e.g. when core's own tests run without cli on the
 * classpath — so tests still see a consistent, cacheable version rather than a moving target.
 */
private fun buildVersion(): String {
    ...
    AnalysisCache::class.java.classLoader
        ...
        ?.use { props.load(it) }
    return props.getProperty("version", UNKNOWN_BUILD_VERSION)
}

/**
 * Deterministic fingerprint of the config fields that change what a rule reports:
 * enabled languages, exclude patterns, per-rule overrides, severity/confidence gates,
 * heavyweight types, call depth, and whether tests are included.
 */
private fun configFingerprint(config: AnalysisConfig?): String {
    if (config == null) return NO_CONFIG_FINGERPRINT
    return buildString {
        append("languages=").append(config.languages.map { it.name }.sorted())
        append(";exclude=").append(config.excludePatterns.sorted())
        append(";overrides=")
        append(
            config.ruleOverrides.toSortedMap().entries.joinToString(",") { (id, override) ->
                "$id:${override.enabled}:${override.severity}"
            },
        )
        append(";minSeverity=").append(config.minSeverity)
        append(";minConfidence=").append(config.minConfidence)
        append(";heavyweightTypes=").append(config.heavyweightTypes.sorted())
        append(";maxCallDepth=").append(config.maxCallDepth)
        append(";includeTests=").append(config.includeTests)
    }
}
