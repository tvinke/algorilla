...
public object MethodPurity {
    ...
    private val PURE_PREFIXES = listOf(
        "get", "find", "compute", "calculate", "parse", "format", "convert",
        "resolve", "to", "is", "has", "can", "should", "matches", "contains",
        "compare", "equals", "hash", "size", "length", "count", "index",
        "check", "validate", "determine", "derive", "extract", "build",
        "create", "make", "of", "from", "with",
    )

    // Methods starting with these prefixes are likely side-effectful
    private val SIDE_EFFECT_PREFIXES = listOf(
        "set", "add", "put", "remove", "delete", "clear", "reset",
        "write", "save", "persist", "store", "insert", "update",
        "send", "emit", "dispatch", "publish", "notify", "fire",
        "log", "print", "debug", "info", "warn", "error", "trace",
        "import", "export", "upload", "download",
        "close", "shutdown", "destroy", "dispose",
        "assert", "verify", "expect", "should", "require",
        "throw", "fail",
     ...
     */
    public fun classify(methodName: String, qualifiedTarget: String?): Purity {
     ...
     */
    public fun isSideEffect(methodName: String, qualifiedTarget: String? = null): Boolean =
        classify(methodName, qualifiedTarget) == Purity.SIDE_EFFECT
}
